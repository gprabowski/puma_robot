...
float vector4::lengthSqr() const { return (x * x + y * y + z * z + w * w); }
...
matrix33 &matrix33::invert() {
  ...
  float determine = a[0][0] * a[1][1] * a[2][2] + a[0][1] * a[1][2] * a[2][0] +
                    a[0][2] * a[1][0] * a[2][1] - a[2][0] * a[1][1] * a[0][2] -
                    a[2][1] * a[1][2] * a[0][0] - a[2][2] * a[1][0] * a[0][1];
...
matrix44::matrix44(const vector4 &v0, const vector4 &v1, const vector4 &v2,
                   const vector4 &v3) {
  col[0] = v0;
  col[1] = v1;
  col[2] = v2;
...
matrix44 OrthoNormalMatrix44(const vector3 &xdir, const vector3 &ydir,
                             const vector3 &zdir) {
  matrix44 ret;
