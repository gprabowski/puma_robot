...
void framebuffer::invalidate() {
  ...
  // depth
  glTextureParameteri(of_fb_dep_tex, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
  glTextureParameteri(of_fb_dep_tex, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  glTextureParameteri(of_fb_dep_tex, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  glTextureParameteri(of_fb_dep_tex, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
  ...
  if (glCheckNamedFramebufferStatus(of_fb, GL_FRAMEBUFFER) !=
    ...
    GK2_PUMA_CRITICAL("Framebuffer creation failed!");
  }
